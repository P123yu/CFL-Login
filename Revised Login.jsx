...
import { useStoreUpdatePwdRolePopup } from "../component/ZustandLocalStorage";
...
function Login() {
  ...
  const { pwdRole, setPwdRole } = useStoreUpdatePwdRolePopup();
  ...
  const sendOtp = () => {
    ...
    axios
      .post(
        `http://localhost:8080/cfl/generate-otp/${forgotPasswordEmail}/${pwdRole}`
      )
      .then((res) => {
  ...
  const resetPassword = () => {
    axios
      .post(
        `http://localhost:8080/cfl/update-password/${forgotPasswordEmail}/${newPassword}/${pwdRole}`
      )
      .then((res) => {
